...
@Component({
  ...
  template: `
    ...
    <div class="grid-table-item pointer" (click)="toggleFolder.emit()">
      <!-- Whitespace -->
      <baw-meta-review-whitespace [indentation]="row.indentation" [isFolder]="row.isOpen"></baw-meta-review-whitespace>
      <fa-layers class="fa-custom-icon me-3" [fixedWidth]="true">
        <fa-icon [icon]="row.isOpen ? icons.folderOpen : icons.folderClosed"></fa-icon>
        @if (!row.isRoot) {
          <fa-layers-counter
      ...
      @if (!row.isRoot) {
        <div class="icon-wrapper">
          @if (harvestItem.hasItemsInvalidFixable) {
            <span class="badge text-bg-warning pointer" (click)="toggleFolder.emit()">
              <fa-icon [icon]="icons.warning"></fa-icon>
              {{ report.itemsInvalidFixable | number }}
            </span>
          }
          @if (harvestItem.hasItemsInvalidNotFixable) {
            <span class="badge text-bg-danger text-light pointer" (click)="toggleFolder.emit()">
...
export class FolderRowComponent {
  ...
  public constructor(@Inject(ASSOCIATION_INJECTOR) protected injector: AssociationInjector) {}
