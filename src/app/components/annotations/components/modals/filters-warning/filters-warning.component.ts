...
@Component({
  ...
  template: `
    ...
    <div class="modal-body">
      <p>You are seeing <em>all</em> ({{ itemCount | number }}) annotations because no search filters were used.</p>

      <p>
        You can verify all the annotations in one go, but it may take a long time to complete and it will be difficult
        to navigate.
      </p>
      ...
      <div class="clearfix">
        <button class="btn btn-outline-primary float-start" (click)="closeModal(false)">Cancel</button>

        <button class="btn btn-primary float-end" (click)="closeModal(true)">Proceed</button>
      </div>
    </div>
  `,
  imports: [DecimalPipe],
