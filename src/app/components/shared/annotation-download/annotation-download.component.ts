...
import { hasResolvedSuccessfully, retrieveResolvedModel, retrieveResolvers } from "@baw-api/resolver-common";
...
@Component({
  ...
  template: `
    <div class="modal-header">
      ...
      <button type="button" class="btn-close" aria-label="Close" (click)="dismissModal('Exit')"></button>
    </div>
    ...
    <div class="modal-body">
      <baw-form [submitLabel]="null" [model]="model" [fields]="fields">
        <span id="subTitle">
          <p>
            The annotations in the CSV will have all their dates and times set to a time zone of your choice.
            @if (site) {
          ...
          @if (site) {
            <p>
              For example, annotations created for audio from Brisbane will have dates and times set to AEST (+10:00).
            </p>
          }
          <p>If you have recordings from Brisbane and Perth, which time zone do we choose for all downloaded events?</p>
          <p>AEST (+10:00) or AWST (+08:00) or UTC (+00:00)?</p>
          <p>
            It depends on how you want to work with your data, so the choice is yours. Please select the time zone you
            wish to use:
          </p>
        </span>
      </baw-form>
    ...
    <div class="modal-footer">
      <a class="btn btn-primary" [href]="getAnnotationsPath()"> Download Annotations </a>
    </div>
  `,
  styles: [
    `
...
export class AnnotationDownloadComponent extends withUnsubscribe() implements OnInit, ModalComponent {
  ...
  public ngOnInit(): void {
    this.sharedRoute.pageInfo.pipe(takeUntil(this.unsubscribe)).subscribe((pageInfo: IPageInfo) => {
  ...
  public getAnnotationsPath(): string {
    ...
    const timezone = this.model.timezone === "Etc/UTC" ? "UTC" : this.model.timezone;

    if (this.site) {
      return this.siteApi.downloadAnnotations(this.site, this.region?.projectId ?? this.project, timezone);
    }
