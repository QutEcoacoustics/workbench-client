...
export class ToastService {
  ...
  public show(message: string, title = "", options: ToastOptions = {}) {
    this._toasts.update((values) => values.concat({ title, message, options, variant: "default" }));
  }

  public success(message: string, title = "", options: ToastOptions = {}) {
    this._toasts.update((values) => values.concat({ title, message, options, variant: "success" }));
  }

  public error(message: string, title = "", options: ToastOptions = {}) {
    this._toasts.update((values) => values.concat({ title, message, options, variant: "danger" }));
  }

  public info(message: string, title = "", options: ToastOptions = {}) {
    this._toasts.update((values) => values.concat({ title, message, options, variant: "info" }));
  }

  public warning(message: string, title = "", options: ToastOptions = {}) {
    this._toasts.update((values) => values.concat({ title, message, options, variant: "warning" }));
  }
